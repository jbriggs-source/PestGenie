...
struct SafetyChecklistView: View {
    ...
    @State private var selectedCategory: SafetyChecklistCategory = .ppe
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        VStack(spacing: 8) {
                            DetailRow(title: "Category", value: violation.category.displayName)
                            DetailRow(title: "Reported", value: violation.reportedAt.formatted())
                            DetailRow(title: "Status", value: violation.isResolved ? "Resolved" : "Active")

                            if violation.isResolved, let resolvedAt = violation.resolvedAt {
                                DetailRow(title: "Resolved", value: resolvedAt.formatted())
                            }
                        }
                    }
